...
public class VisualShaderNodeResizableBase extends VisualShaderNode {
	private static final Map<String, Long> METHOD_HASHES = java.util.Map
			.ofEntries(java.util.Map.entry("set_size", 743155724L), java.util.Map.entry("get_size", 3341600327L));

	VisualShaderNodeResizableBase(MemorySegment nativePointer) {
		super(nativePointer);
	}

	VisualShaderNodeResizableBase(long nativePointer) {
		super(nativePointer);
	}
