...
public class NavigationPathQueryResult2D extends RefCounted {
	private static final Map<String, Long> METHOD_HASHES = java.util.Map.ofEntries(
			java.util.Map.entry("get_path", 2961356807L), java.util.Map.entry("set_path_rids", 381264803L),
			...
			java.util.Map.entry("set_path", 1509147220L));

	NavigationPathQueryResult2D(MemorySegment nativePointer) {
		super(nativePointer);
	}

	NavigationPathQueryResult2D(long nativePointer) {
		super(nativePointer);
	}
